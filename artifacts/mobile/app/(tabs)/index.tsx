...
export default function ReelsScreen() {
  ...
  // Reel deep-link: tapping a reel on a profile navigates here with startReelId.
  // We pin that reel at index 0 so it plays immediately (TikTok pattern).
  const { startReelId, userId: startUserId } = useLocalSearchParams<{ startReelId?: string; userId?: string }>();
  const [pinnedReel, setPinnedReel] = useState<Reel | null>(null);

  useEffect(() => {
    if (!startReelId) return;
    const API_BASE = (process.env["EXPO_PUBLIC_API_URL"] ?? "") + "/api";
    (async () => {
      try {
        let data: any = null;
        // Primary: dedicated endpoint (works once server is updated)
        const res = await fetch(`${API_BASE}/reels/${encodeURIComponent(startReelId)}`);
        if (res.ok) {
        ...
        if (!data && startUserId) {
          const fbRes = await fetch(`${API_BASE}/posts/user/${encodeURIComponent(startUserId)}`);
          if (fbRes.ok) {
            const fbBody = await fbRes.json();
            data = (fbBody.reels as any[] ?? []).find((r: any) => r.id === startReelId) ?? null;
          }
        }
        if (!data) return;
        const pinned: Reel = {
        ...
        if (__DEV__) console.warn("[Reels] startReelId fetch failed:", e?.message);
      }
    })();
  }, [startReelId, startUserId]);
